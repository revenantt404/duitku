    ...
    <Card className={cn("card-hover min-w-0", className)}>
      <CardContent className="p-4 min-w-0">
        <div className="min-w-0">
          ...
          <div className="text-[11px] tracking-wide text-mute dark:text-[#8f8b85] truncate">{wallet.type}</div>
        </div>
        <div className={`mt-3 text-[18px] font-semibold tracking-tight leading-none num truncate min-w-0 ${state === "bad" ? "text-[#b42318] dark:text-[#fca5a5]" : state === "warn" ? "text-[#a16207] dark:text-[#fcd34d]" : "text-[#1a7a4a] dark:text-[#4ade80]"}`}>{formatRupiah(balance)}</div>
